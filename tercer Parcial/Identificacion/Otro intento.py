# -*- coding: utf-8 -*-
"""
Created on Tue Apr  8 07:45:06 2025
    ...
    def run(self):
        ...
        while self.hilo:
            ...
            if ret:
                ...
                p = conversion.scaled(300, 200, QtCore.Qt.KeepAspectRatio)
                self.ImagenC.emit(p)
            ...
            if ret:
                ...
                if self.bandera = False 
                    ...
                    for (top, right, bottom, left), cara_frame in zip(ubicaciones, codigos):
                        #encontradas = face_recognition.compare_faces(self.caras, cara_frame)
                        nombre = "Desconocido"
                        #if True in encontradas:
                            if self.caras:
                                distacia = face_recognition.face_distance(self.caras,codificacion)
                                min_distacia = min(distacia)
                                indice_min= distacia.tolist().index(min_distacia)
                                if min_distacia <0.5:
                                    nom ='Hola'+self.nombre[indice_min]
                                    self.lblMostrarNombre.emit(nom)
                                else:
                                    nom = "Desconocido"
                                    self.lblMostrarNombre.emit(nom)
